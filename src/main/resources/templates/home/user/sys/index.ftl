...
layui.use(['element', 'table', 'layer'], function(){
  ...
  table.render({
    ...
    cols: [[
      {type:'checkbox'},
      {field:'ucode', width:280, title: '账号编码'},
...
function editUser(ucode) {
	...
	layer.open({
	  ...
	  fixed: false, //不固定
	  maxmin: true,
	  content: ctx + baseUrl + "/edit.do?ucode="+ucode,
	});
}
